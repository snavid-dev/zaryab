...
export default function Genre({ title }) {
  return (
    <div className="relative flex group h-21px">
      <div className="font-common-thin z-20 text-18px text-right">{title}</div>
      <svg
        width="21"
        height="21"
        viewBox="0 0 21 21"
        ...
        className="absolute right-0 z-10 group-hover:rotate-90 transition-all duration-1000 mt-1"
      >
        <path
      ...
      <div className="absolute right-0 opacity-0 bg-[#FCA30A] w-[21px] h-[21px] rounded-full group-hover:opacity-100 transition-all duration-500 delay-500 mt-1"></div>
    </div>
  );
}
